    ...
    internal class StateSet
    {
        ...
        private static void ChangeLevel()
        {
            ...
            if (Kyoshin.IsTriggerOn) //1点赤ではレベルを変更しない
            {
                ...
                {
                    Instance.GroupBox_Kyoshin.BorderColor = Util.Red;
                }
                else if (Instance.Label_KyoshinMaxInt.Text.Contains("3（") || Instance.Label_KyoshinMaxInt.Text.Contains("4（"))
                {
                    Instance.GroupBox_Kyoshin.BorderColor = Util.Yellow;
                }
                else
                {
                    Instance.GroupBox_Kyoshin.BorderColor = Util.White;
                }
            }
            else
            {
            ...
            if (Eew.IsTriggerOn)
            {
                ...
                {
                    Instance.GroupBox_Eew.BorderColor = Util.Red;
                }
                else if (Instance.Label_EewMessage.Text.Contains("予報"))
                {
                    Instance.GroupBox_Eew.BorderColor = Util.Yellow;
                }
            }
            else
            {
                Instance.GroupBox_Eew.BorderColor = Util.White;
            }
            ...
            {
                Instance.GroupBox_EqInfo.BorderColor = Util.Red;
            }
            else if (EqInfo.MaxInt.Contains("3") || EqInfo.MaxInt.Contains("4") || Instance.Label_EqInfoComment.Text.Contains("注意報"))
            {
                Instance.GroupBox_EqInfo.BorderColor = Util.Yellow;
            }
            else
            {
                Instance.GroupBox_EqInfo.BorderColor = Util.White;
            }
        }
